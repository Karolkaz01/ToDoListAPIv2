...
namespace ToDoListAPI.Services
{
    public class NoteService : INoteService
    {
        ...
        {
            //var newID = _notes.Select(x => x.Id).DefaultIfEmpty(0).Max() + 1;
            //var newNote = new Note() { NoteValue = note.NoteValue, Title = note.Title , Id = newID , CreateDate = DateTime.Now };
            //_notes.Add(newNote);
            //return newNote;
            var response = await _dbContext.Notes.AddAsync(new Note() { NoteValue = note.NoteValue, Title = note.Title , CreateDate = DateTime.UtcNow});
            _dbContext.SaveChanges();
